...
public class LANChaosKiller extends Script implements Painting, MouseActions 
{
	...
	public void run() 
	{
		try 
		{
			...
			{
				stateMap.put(state, LANChaosKiller.class.getMethod(state.name()));
			}
	...
	private State getState() 
	{
		if (Inventory.isFull() || (foodCount > 0 &&  Inventory.find(foodID).length == 0 && (player.getMaxHealth() / 2) >= player.getHealth()))
		{
			...
			{
				// We are infront of at the bank and in need of some banking action.
	...
	public static void PROCESS_DRUIDS() 
	{
		if (foodCount > 0 && (player.getMaxHealth() / 3) >= player.getHealth() && Inventory.find(foodID).length > 0)
		{
			...
			// Double inventory.find call but since 99% of the time we wont reach these conditions it's better performance to do the double call now.
		...
		if (!player.isInCombat())
		{
			...
			RSNPC[] druids = NPCs.find(NPCs.generateFilterGroup(new Filter<RSNPC>(){
				@Override
				public boolean accept(RSNPC npc) {
					return !npc.isInCombat() && npc.getName() == "Chaos druid";
				}
			}));
		...
		{
			// While we attack a druid we'll prepare for the next one.(If there are more)
			...
			for (int i = 0; i < druids.length; i++)
			{
				if (PathFinding.canReach(druids[i], false))
				{
					...
					while (player.isInCombat() && druids[i].isValid() && !druids[i].isInCombat())
					{
						if (!druids[i].isInCombat())
						{
							druids[i].hover();
